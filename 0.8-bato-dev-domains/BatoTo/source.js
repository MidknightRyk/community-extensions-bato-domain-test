...
var _Sources = (() => {
  ...
  var BTDomainsClass = class {
    ...
    getDefault() {
      return this.Domains.filter((Domain) => Domain.default).map((Domain) => Domain.url) ?? ["https://bato.si"];
    }
  };
  ...
  var parseChapterList = ($, mangaId) => {
    ...
    for (const chapter of $('[q\\:key="N0_9"]').toArray()) {
      ...
      const chapterId = $("a", chapter).first().attr("href")?.replace(/\/$/, "")?.split("/").pop() ?? "";
      const groupDiv = $('[q\\:key="00_1"]', chapter).first();
      const group = $("a", groupDiv).last().text().trim();
  ...
  var parseChapterDetails = ($, mangaId, chapterId) => {
    ...
    const pages = pagesDivs.map((pageDiv) => {
  ...
  var parseHomeSections = ($, baseURL, sectionCallback) => {
    ...
    for (const manga of $('[q\\:key="85_7"]').toArray()) {
      const image = baseURL + $("img", manga).attr("src");
      const title = $("img", manga).attr("title")?.trim() ?? "";
      ...
      const subtitle = lang + " " + $("a", $("div", manga).first()).last().text().trim();
    ...
    for (const manga of $('[q\\:key="wP_7"]').toArray()) {
      ...
      const image = baseURL + $("img", mainDiv).attr("src");
      const title = $("img", mainDiv).first().attr("title")?.trim() ?? "";
      const id = $("a", mainDiv).attr("href")?.replace("/title/", "")?.trim().split("/")[0] ?? "";
      const btcode = $("em", mainDiv).attr("data-lang");
      const lang = btcode ? BTLanguages.getLangCode(btcode) : "\u{1F1EC}\u{1F1E7}";
      const subtitle = lang + " " + $("a", subDiv).first().text().trim();
      if (!id || !title) continue;
